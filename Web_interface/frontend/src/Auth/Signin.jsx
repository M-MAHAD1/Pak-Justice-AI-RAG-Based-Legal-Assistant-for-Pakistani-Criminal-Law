...
import { parseJwt, setAuth } from "../utils/auth";
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      setAuth(data?.token);
            ...
            navigate(claims?.role === "admin" ? "/admin" : "/");
    } catch (err) {
        ...
        <div className="relative z-10 min-h-screen flex items-center justify-center pt-28 md:pt-32 pb-16 px-4">
            ...
            >
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-[color:var(--text-h)]">Welcome Back</h2>
                    <p className="text-[color:var(--text)] text-sm mt-2">Sign in to access pakJustice AI</p>
                </div>
                ...
                <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--bg)]/70 backdrop-blur-xl p-8 shadow-2xl shadow-cyan-500/5">
                    ...
                    <form onSubmit={handleSignIn} className="space-y-5">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center">
                                <input
                                    type="checkbox"
                                    id="remember"
                                    className="w-4 h-4 rounded border-[color:var(--border)] bg-[color:var(--code-bg)] text-cyan-500 focus:ring-cyan-500 focus:ring-offset-[color:var(--bg)]"
                                />
                                <label htmlFor="remember" className="ml-2 text-xs text-[color:var(--text)]">
                                    Keep me signed in
                                </label>
                            </div>
                        </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className={`w-full flex items-center justify-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-white font-semibold py-3 rounded-lg transition-all active:scale-[0.98] shadow-lg shadow-cyan-500/10 ${loading ? "opacity-70 cursor-not-allowed" : ""}`}
                        >
                            {loading ? (
                ...
                <div className="mt-6 text-center">
                    ...
                    <p className="text-xs text-[color:var(--text)] mt-2">
                        Need help?{" "}
